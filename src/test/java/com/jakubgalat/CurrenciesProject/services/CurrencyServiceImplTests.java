...
class CurrencyServiceImplTests {
	...
	public void testGetCurrentCurrencyValueCommand(){
		...
		try{
			doReturn("currenciesString").when(currencyService).makeGetRequestAndGetCurrenciesString(any(),any());
			when(objectMapper.readValue("currenciesString",CurrencyTableA.class)).thenReturn(new CurrencyTableA());
